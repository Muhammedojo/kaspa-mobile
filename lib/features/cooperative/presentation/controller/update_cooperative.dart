...
import '../view/update_cooperative.dart';

class UpdateCooperativeScreen extends StatefulWidget {
  final Cooperative? cooperative;
  const UpdateCooperativeScreen({super.key, this.cooperative});

  @override
  State<UpdateCooperativeScreen> createState() =>
      _UpdateCooperativeScreenState();
}

class _UpdateCooperativeScreenState extends State<UpdateCooperativeScreen>
    implements UpdateCooperativeControllerContract {
  late final UpdateCooperativeViewContract view;
  ...
  void initState() {
    ...
    final lgaDataFromCooperative = cooperative?.lga;
    if (lgaDataFromCooperative != null) {
      selectedLga =
          Lga()
            ..pk = lgaDataFromCooperative.id!
            ..name = lgaDataFromCooperative.name;
    } else {
      selectedLga = null;
    }

    final wardDataFromCooperative = cooperative?.ward;
    if (wardDataFromCooperative != null) {
      selectedWard =
          Ward()
            ..pk = wardDataFromCooperative.id!
            ..name = wardDataFromCooperative.name;
    } else {
      selectedWard = null;
    }

    nameController = TextEditingController(text: cooperative?.name ?? '');
    certificateNumberController = TextEditingController(
      text: cooperative?.certificateNumber ?? '',
    );
    doiController = TextEditingController(text: cooperative?.doi ?? '');
    imageController = TextEditingController(text: cooperative?.file ?? '');

    picker = ImagePicker();

    view = UpdateCooperativeView(controller: this);
  }
